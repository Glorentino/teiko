...
if __name__ == "__main__":
    ...
    ft.to_csv(OUTPUT_DIR / "frequency_table.csv", index=False)
    print(f"Saved -> {OUTPUT_DIR / 'frequency_table.csv'}")

    print("\n=== Part 3: Statistical Analysis ===")
    st = run_statistics()
    print(st.to_string(index=False))
    st.to_csv(OUTPUT_DIR / "stats_results.csv", index=False)
    print(f"Saved -> {OUTPUT_DIR / 'stats_results.csv'}")

    print("\n=== Part 3: Box Plot ===")
    fig = make_boxplot()
    fig.savefig(OUTPUT_DIR / "boxplot.png", dpi=150, bbox_inches="tight")
    plt.close(fig)
    ...
    subset["samples"].to_csv(OUTPUT_DIR / "baseline_subset.csv", index=False)
    subset["samples_per_project"].to_csv(OUTPUT_DIR / "baseline_samples_per_project.csv", index=False)
    subset["response_counts"].to_csv(OUTPUT_DIR / "baseline_response_counts.csv", index=False)
    subset["sex_counts"].to_csv(OUTPUT_DIR / "baseline_sex_counts.csv", index=False)
    print(f"Saved -> {OUTPUT_DIR / 'baseline_*.csv'}")
